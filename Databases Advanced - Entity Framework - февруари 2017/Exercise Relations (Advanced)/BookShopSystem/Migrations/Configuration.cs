namespace BookShopSystem.Migrations
{
    ...
    public class Configuration : DbMigrationsConfiguration<BookShopSystem.Data.BookShopContext>
    {
        ...
        {
            string[] authors = File.ReadAllLines(@"c:\users\admin\documents\visual studio 2015\Projects\BookShopSystem\BookShopSystem\Import\authors.csv");
        ...
        private static void SeedBooks(BookShopContext context)
        {
            ...
            string[] books = File.ReadAllLines(@"c:\users\admin\documents\visual studio 2015\Projects\BookShopSystem\BookShopSystem\Import\books.csv");
            ...
            for (int i = 1; i < books.Length; i++)
            {
                ...
                EditionType edition = (EditionType)n;
                DateTime releaseDate = DateTime.ParseExact(data[1], "d/MM/yyyy", CultureInfo.InvariantCulture);
                int copies = int.Parse(data[2]);
                decimal price = decimal.Parse(data[3].Replace(".",","));
        ...
        {
            context.Categories.Add(new Models.Category { Name = "Botev" });
            context.SaveChanges();
            int booksCount = context.Books.Local.Count;
            string[] categories = File.ReadAllLines(@"c:\users\admin\documents\visual studio 2015\Projects\BookShopSystem\BookShopSystem\Import\categories.csv");
